...
continuity-auth.server.identity.{score,merge}   trust math (incl. v7 IP-bounce velocity + durable strikes) + classification
continuity-auth.server.ratelimit.{tier,window}  tiering (incl. v7 IP-bounce hard floor) + windowed counter
...
- **IP-bounce penalty: durable, fp-gated velocity strike with a tier hard-floor (v7).** A same-key, same-fingerprint identity rotating across many IPs at high frequency (the rotating-residential-proxy signature) is detected by a recency-weighted IP-change *velocity* (`ip-churn`, ~1h half-life) added to the per-identity sketch; crossing `:churn-strike-threshold` accrues a **durable** `:identity/ip-bounce-strikes` (≤ one per 6h cooldown) that decays only on a slow 14d half-life. Locked choices: (1) **velocity, not lifetime distinct-IP count** — a legit roamer accumulates many IPs over time; the discriminator is *rate of change*, fp-gated so a genuine device+network change (the `:fp` axis) accrues nothing; (2) **durable strike + hard tier-floor, not just score erosion** — strikes both multiply the score down (`bpen` in `score-of`) *and* cap the tier at `:penalized` in `tier/project`, bypassing `earned` so an aged, high-`clean-count` identity cannot dilute the penalty with cheap clean volume (the asymmetry the `vrate`-only term got backwards — this realises slow-up / fast-down); (3) **global, not per-verifier** — the signal is continuity-auth's *own* observation (the IP on every verify), not a host-reported verdict, so it lives in the global sketch and is not a portable scarlet letter (the verifier-scoped-state fork stays deferred, needed only for host-reported abuse); (4) **conservative threshold** (~1 IP change/85s sustained) keeping the shared-NAT/mobile population the project protects clear by 1.5–2 orders of magnitude. Additive `schema-version` 6→7 (five sparse Identity attrs; absent ⇒ pure v6 behaviour). A strike writes a `:trust-event` `:ip-bounce` for audit; the verify response shape and hot path are unchanged. See ontology §6, threat-model T22.
