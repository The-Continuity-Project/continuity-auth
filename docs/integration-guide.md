...
- **No shared cookies.** continuity-auth never sets cookies. Storage is IndexedDB on the client side.
- **No session passing.** continuity-auth has no concept of "logged-in". It has an `identity_ref` to correlate with the host's own user record.
- **No proxying through the host domain.** Direct connection from the browser to fl-endpoint is fine and reduces latency.

## Worth considering

- **Caching the verify response** when making multiple decisions about the same envelope is unnecessary. Each envelope has a unique nonce and is single-use.
- **Pre-warm the keypair** by calling `cauth.init()` early in app boot, so the first user action doesn't pay keygen latency.
- **Expose a `Retry-After` based on continuity-auth's `retry_after_ms`** so the frontend can render a sensible "please wait" UX.
